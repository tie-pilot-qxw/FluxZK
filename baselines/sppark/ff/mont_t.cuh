...
template <size_t N>
__host__ void host_random_raw(uint32_t *r, const uint32_t *m)
{
    if constexpr (N == 0)
    return;
    else
    ...
    uint32_t x = (uint32_t)std::rand() % m[N - 1];
    r[N - 1] = x;
    if (x == m[0])
    {
        return host_random_raw<N - 1>(r, m);
    }
...
public:
    ...
    static __host__ inline mont_t host_random() {
        mont_t r;
        host_random_raw<n>(r.even, MOD);
        return r;
    }
