...
extern "C"
void* init_msm(const affine_t points[], size_t npoints)
{

    auto msm = new msm_t<bucket_t, point_t, affine_t, scalar_t>{points, npoints};
    return (void*)msm;
}

extern "C"
RustError::by_value mult_msm(void* msm_ptr, point_t* out, const scalar_t scalars[],
                             size_t ffi_affine_sz)
{
    auto msm = (msm_t<bucket_t, point_t, affine_t, scalar_t>*)msm_ptr;
    std::cout << "Number Points: " << msm->npoints << std::endl;

    return msm->invoke(
        *out, nullptr, msm->npoints, scalars, false, ffi_affine_sz
    );
}
