...
public:
        ...
        : gpu(select_gpu(device_id)), d_points(nullptr), d_scalars(nullptr)
    {
        ...
        if (points) {
            ...
            auto all_time = std::chrono::duration_cast<std::chrono::milliseconds>(all_end - all_begin);
            std::cout << "Transfer points: " << all_time.count() << " ms" << std::endl;
            npoints = np;
        } else {
            npoints = 0;
        }
...
public:
                                   ...
                                   size_t ffi_affine_sz = sizeof(affine_t))
    {
        ...
        float transfer_time = 0, point_transfer_time = 0;
        float comp_time = 0;
        float time;
        cudaEvent_t transfer_start, transfer_end, comp_start, comp_end;
        ...
        try {
            ...
            if (scalars) {
                ...
                cudaEventRecord(transfer_start, gpu[2]);
                gpu[2].HtoD(&d_scalars[d_off], &scalars[h_off], num);
                cudaEventRecord(transfer_end, gpu[2]);
#ifdef MEASURE_TIME
                cudaEventSynchronize(transfer_end);
#endif

                cudaEventElapsedTime(&time, transfer_start, transfer_end);
                transfer_time += time;
            ...
            cudaEventRecord(comp_start, gpu[2]);

            digits(&d_scalars[0], num, d_digits, d_temps, mont);
            gpu[2].record(ev);

            cudaEventRecord(comp_end, gpu[2]);
#ifdef MEASURE_TIME
            ...
            if (points) {
                ...
                cudaEventRecord(transfer_start, gpu[0]);

                gpu[0].HtoD(&d_points[d_off], &points[h_off],
                            num,              ffi_affine_sz);

                cudaEventRecord(transfer_end, gpu[0]);
            ...
            for (uint32_t i = 0; i < batch; i++) {
                ...
                cudaEventRecord(comp_start, gpu[i&1]);
                ...
                cudaEventRecord(comp_end, gpu[i&1]);
#ifdef MEASURE_TIME
                cudaEventSynchronize(comp_end);
#endif
                cudaEventElapsedTime(&time, comp_start, comp_end);
                ...
                if (i < batch-1) {
                    ...
                    if (points) {
                        ...
                        cudaEventRecord(transfer_start, gpu[j]);

                        gpu[j].HtoD(&d_points[d_off], &points[h_off*ffi_affine_sz],
                                    num,              ffi_affine_sz);

                        cudaEventRecord(transfer_end, gpu[j]);
