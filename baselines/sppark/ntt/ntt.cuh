...
public:
                          ...
                          Type type)
    {
        ...
        try {
            ...
            cudaEvent_t compute_begin, begin , compute_end, end;
            cudaEventCreate(&compute_begin);
            cudaEventCreate(&begin);
            cudaEventCreate(&compute_end);
            cudaEventCreate(&end);

            cudaEventRecord(begin, gpu);
            gpu.HtoD(&d_inout[0], inout, domain_size);

            cudaEventRecord(compute_begin, gpu);
            NTT_internal(&d_inout[0], lg_domain_size, order, direction, type, gpu);
            cudaEventRecord(compute_end, gpu);

            gpu.DtoH(inout, &d_inout[0], domain_size);
            cudaEventRecord(end, gpu);
            gpu.sync();

            float compute_t, total_t;

            cudaEventElapsedTime(&compute_t, compute_begin, compute_end);
            cudaEventElapsedTime(&total_t, begin, end);

            cudaEventDestroy(compute_begin);
